...
| [`.env.example`](.env.example) | template | Copy to **`LLM/.env`** for API keys |
| [`openai_env.py`](openai_env.py) | helper | Loads `OPENAI_API_KEY` from `LLM/.env` |
...
- **`LLM/.env`** — `OPENAI_API_KEY` (copy from `.env.example`)
- **`LLM/.venv`** — Python packages for all modules

## 2025 reference

Archive (Elasticsearch homework, original Introduction): [`../2025/`](../2025/)

Homework compose (2025): `../2025/Introduction/llm_01_hw/docker-compose.yml`
