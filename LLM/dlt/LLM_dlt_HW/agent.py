...
def build_model():
    """Use Cerebras + Gemma (course default), OpenAI-compatible API."""
    backend = os.getenv("LLM_BACKEND", "cerebras").strip().lower()
    if backend == "cerebras":
    ...
    # Fallback: stock OpenAI model string from homework starter
    return os.getenv("OPENAI_MODEL", "openai:gpt-5.4-mini")
